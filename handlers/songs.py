...
@Client.on_message(command(["find", f"find@{bn}"]) & ~filters.edited)
def bul(_, message):
    ...
    try:
        ...
        print(str(e))
        return
    m.edit("📥 İndirme işlemi başlatıldı...")
    try:
...
@Client.on_message(
    command(["vfind", f"vfind@{bn}", "video", f"video@{bn}"]) & ~filters.edited
)
...
async def lyrics(_, message):
    try:
        if len(message.command) < 2:
            await message.reply_text("» **lirik bir isim de vermek.**")
            return
        query = message.text.split(None, 1)[1]
        rep = await message.reply_text("🔎 **şarkı sözlerini arama...**")
        resp = requests.get(
